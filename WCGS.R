...
```
Initially, **cholesterol had 12 missing values (0.4%)**, which were removed prior to analysis and a table of missing values is shown below for clarity.
...
```
Although **weight and height** were not statistically significant (p > 0.05).

## Forward Selection Based on AIC
As a complementary approach, we also applied *forward selection*, starting from a null model and adding predictors step by step based on improvement in AIC:
```{r}
...
To choose the final model, I compared several versions based on AIC and prediction accuracy.
...
During forward selection, **weight** was the last variable added. Although it wasn’t statistically significant, including it didn’t meaningfully improve AIC or prediction accuracy.

In backward selection,**'weight' and 'height'** were not statistically significant either, but keeping them was beneficial. Their inclusion didn’t substantially increase AIC, but the model’s predictive performance improved with them.
...
This table shows the p-values for each predictor across outcome groups compared to the reference. Lower p-values (< 0.05) indicate significant effects. For instance, age is significant in all groups, while weight is not. This helps identify key predictors distinguishing the groups.
...
# Build a data frame to compare AIC and prediction accuracy
...
model_comp %>%
  kbl(caption = "Comparison of Models Based on AIC and Prediction Accuracy") %>%
  kable_styling(full_width = FALSE, bootstrap_options = c("striped", "hover")) %>%
  row_spec(5, background = "#d4edda")  # Highlight the Final Model row in light green
```

We evaluated multiple models to identify the best fit for predicting behavioral patterns. The Full Model, which includes all predictors (age, sbp, dbp, chol, height, weight), had an AIC of 7407.97 and an accuracy of about 44.97%. Removing weight slightly improved the AIC to 7403.13 and accuracy to 45.04%, while removing height or both height and weight did not lead to meaningful improvements.
...
# Extract coefficients and standard errors only for group A2 vs A1
coefs_A2 <- coefs["A2", ]
ses_A2 <- ses["A2", ]

df_A2 <- data.frame(
  variable = names(coefs_A2),
  estimate = coefs_A2,
  se = ses_A2
) %>%
  mutate(
...
**AGE** is statistically significant in the model. Individuals in 'group A2' are approximately 2% **younger** than those in the reference 'group A1'.

**SBP** is also a significant variable. Systolic blood pressure in 'group A2' is about 1% **higher** than in 'group A1'.

**DBP** is statistically significant as well, showing around 2% lower values in group A2 compared to A1.

Other variables, including *cholesterol, height, and weight, were not statistically significant*.
...
# Extract coefficients and standard errors for group B3 vs A1
coefs_B3 <- coefs["B3", ]
ses_B3 <- ses["B3", ]

df_B3 <- data.frame(
  variable = names(coefs_B3),
  estimate = coefs_B3,
  se = ses_B3
) %>%
  mutate(
...
ggplot(df_B3, aes(x = estimate, y = variable)) +
  geom_point(color = "#D55E00", size = 2) +          # Different color to distinguish groups
  geom_errorbarh(aes(xmin = conf.low, xmax = conf.high), height = 0.2, color = "#D55E00") +
  geom_vline(xintercept = 0, linetype = "dashed", color = "gray50") +
  theme_minimal(base_size = 13) +
  labs(
    title = "Coefficient Plot for B3 vs A1",
    x = "Log-Odds Estimate (± 95% CI)",
    y = "Predictors"
  )
```
...
**Cholesterol** is also statistically significant in this comparison, with individuals in 'group B3' showing about 3% **lower** cholesterol levels compared to 'group A1'.

**DBP** is close to the threshold of significance and is about 2% **lower** in 'group B3' than in 'group A1'.

Other variables, including *SBP, height, and weight, were not statistically significant*.
...
# Extract coefficients and standard errors for group B4 vs A1
coefs_B4 <- coefs["B4", ]
ses_B4 <- ses["B4", ]

df_B4 <- data.frame(
  variable = names(coefs_B4),
  estimate = coefs_B4,
  se = ses_B4
) %>%
  mutate(
...
ggplot(df_B4, aes(x = estimate, y = variable)) +
  geom_point(color = "#0072B2", size = 2) +          # Different color for B4
  geom_errorbarh(aes(xmin = conf.low, xmax = conf.high), height = 0.2, color = "#0072B2") +
  geom_vline(xintercept = 0, linetype = "dashed", color = "gray50") +
  theme_minimal(base_size = 13) +
  labs(
    title = "Coefficient Plot for B4 vs A1",
    x = "Log-Odds Estimate (± 95% CI)",
    y = "Predictors"
  )
```
...
**Height** is statistically significant as well, with individuals in group B4 being about 2.2% **shorter** than those in group A1.
...
**DBP** is near the significance threshold and is roughly 2.4% **lower** in' group B4 than in A1'.

Variables such as 'SBP and weight were not statistically significant'.

# **Model Evaluation**

We evaluated the model's predictive performance using a confusion matrix:
  
  ```{r conf_matrix_heatmap, echo=TRUE, message=FALSE, warning=FALSE}
